...
## 最终 depth TAR 发布（2026-09-07）

- Linux CPU 锁定环境的完整测试集 **45 项通过**，包含新增 5 项打包测试；最终空间检查调整后重跑打包测试与 Bash 入口均通过。
- 完整临时 fixture 经过迁移、解包、对齐、写回、全媒体检查和整理后，生成两相机 TAR；
  使用原解包器还原至独立目录，所有 PNG SHA-256 与源文件一致，原 PNG、视频、Parquet/meta 和 simulation TAR 均保留。
- 覆盖校验/整理记录缺失、待复核、源数据变化、未知旧包、额外 episode、磁盘不足、
  TAR 损坏、有无凭据的冲突包拒绝、临时包失败不发布、发布后断点恢复和已有包重新核验。
- 另用 720×1280 随机 uint16 PNG 验证跨流缓冲区的大文件读写和 TAR 全文件 SHA-256。
- 实际 Bash/CLI 使用已有 CPU 环境走通失败拦截、打包与恢复；Ruff 和 32 个 Linux Bash 文档代码块语法检查通过。
- 几何通过条件在打包编排 fixture 中模拟；没有声称重新测量了真实外参质量。
  本轮仅在独立临时数据上运行，不改动真实样例，也没有对对方全量数据打包或估算实测吞吐量。
