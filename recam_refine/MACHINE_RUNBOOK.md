...
收尾校验中，每张 PNG 从存储读取一次，在内存完成格式、CRC、解码和数值检查。清理前保留训练文件变更摘要检查，但不再重新全量计算迁移 PNG 的哈希；清理结束不重复遍历训练文件。跨文件系统源副本删除前仍逐文件核对源和最终文件。视频逐帧解码、模态对齐、metadata/stats 和外参审计保留，不将抽样报告标记为全量校验通过。
